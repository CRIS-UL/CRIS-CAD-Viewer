...
export class CadViewer {
  ...
  _initScene() {
    ...
    // Enable ACES Filmic tone mapping for HDR lighting
    this.renderer.toneMapping = THREE.ACESFilmicToneMapping;
    this.renderer.toneMappingExposure = 0.9;
    // Use physically-correct lights so exposure + light units behave predictably
    ...
    // reduce directional lighting to avoid extreme view-dependent highlights
    const DIR_INTENSITY = 0.12;
    const dirs = [
      [1, 1, 1],
      [-1, 1, 1],
      [1, 1, -1],
    ...
    // Apply CSS background gradient (keeps the HDRI hidden)
    this._applyBackground();

    // Load an HDR environment map for realistic reflections (lighting only, no visible background)
    ...
    new RGBELoader().load(
      ...
      (hdrTexture) => {
        ...
        // Do not set scene.background so the CSS gradient remains visible
        hdrTexture.dispose();
    ...
    this.model.traverse((child) => {
      if (child.isMesh) {
        ...
        mats.forEach((m) => {
          ...
          // Render both sides for thin geometry
          m.side = THREE.DoubleSide;

          // If PBR, provide safe defaults and clamp extremes to avoid mirror-like results
          if (m.isMeshStandardMaterial || m.isMeshPhysicalMaterial) {
            if (m.metalness === undefined) m.metalness = 0.0;
            if (m.roughness === undefined) m.roughness = 1.0;

            // Clamp values to reasonable ranges
            m.metalness = Math.min(Math.max(m.metalness, 0.0), 0.9);
            m.roughness = Math.min(Math.max(m.roughness, 0.05), 1.0);

            // Moderate environment intensity so reflections don't dominate
            m.envMapIntensity = (m.envMapIntensity === undefined) ? 0.6 : Math.min(Math.max(m.envMapIntensity, 0.2), 1.5);
          }

          // Nudge extremely dark base colors so lighting can show them
          if (m.color) {
            const sum = (m.color.r || 0) + (m.color.g || 0) + (m.color.b || 0);
            if (sum < 0.06) {
              m.color.setScalar(0.18);
            }
          }

          // Ensure color textures use sRGB encoding
          const setSRGB = (tex) => {
            if (tex && tex.encoding !== undefined) tex.encoding = THREE.sRGBEncoding;
          };
          setSRGB(m.map);
          setSRGB(m.emissiveMap);
          setSRGB(m.specularMap);
          setSRGB(m.lightMap);
          setSRGB(m.aoMap);

          // Hook up the scene environment for IBL reflections
          if (this.scene.environment && (m.isMeshStandardMaterial || m.isMeshPhysicalMaterial)) {
  ...
  _bindExposureKeys() {
    window.addEventListener("keydown", (e) => {
      if (e.key === "=" || e.key === "+") this._changeExposure(0.05);
      if (e.key === "-") this._changeExposure(-0.05);
    });
  }

  _changeExposure(delta) {
    if (!this.renderer) return;
    this.renderer.toneMappingExposure = Math.max(0.2, Math.min(3.0, this.renderer.toneMappingExposure + delta));
    // optional: surface feedback in console
    // eslint-disable-next-line no-console
    console.log("toneMappingExposure", this.renderer.toneMappingExposure.toFixed(2));
  }
